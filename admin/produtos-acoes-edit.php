<?php
    ...
    function update($id, $id_categoria, $nome, $material, $preco, $peso, $link, $texto, $nome_atual){
        $sqlInsere = "UPDATE produtos SET 
        id_categoria='$id_categoria', nome='$nome', material='$material', preco='$preco', peso='$peso', link='$link', texto='$texto', imagem_destaque='$nome_atual'
        WHERE
        id = $id";
        //exit();
    ...
    function updateSemImagem($id, $id_categoria, $nome, $material, $preco, $peso, $link, $texto){
        $sqlInsere = "UPDATE produtos SET 
        id_categoria='$id_categoria', nome='$nome', material='$material', preco='$preco', peso='$peso', link='$link', texto='$texto'
        WHERE
        id = $id;";
        //exit();
        return update_db($sqlInsere);
            ...
            if($tamanho < 512){ //se imagem for até 500KB envia
                ...
                if(move_uploaded_file($tmp,$pasta.$nome_atual)){
                    if(deletaArquivo($id)){
                        ...
                        if(update($id, $id_categoria, $nome, $material, $preco, $peso, $link, $texto, $nome_atual)){
    ...
    if(isset($imagem_destaque) && $imagem_destaque["name"] != ""){
        uploadImg($id, $id_categoria, $nome, $material, $preco, $peso, $link, $texto, $imagem_destaque);
    } else {
        if(updateSemImagem($id, $id_categoria, $nome, $material, $preco, $peso, $link, $texto, $imagem_destaque)){
            echo "<script type='text/javascript'>alert('Operação realizada com sucesso!'); window.location = 'produtos.php';</script>";
            exit();
        }
    }
